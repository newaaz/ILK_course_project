...
SERVICE_TITLES = ["Такси 'Семёрка'", "Служба такси 'Виктор'", "Детская комната 'Одуванчик'", "Ремонт обуви", "Ремонт смартфонов",
                   "Спортзал 'Аполлон'", "Маникюр/педикюр", "Массаж", "Доставка еды 'БыстроПицца'", "Прокат авто", "Услуги эваукатора",
                   "Бильярд 'Кий'", "Теннис", "Сауна, русские и турецкие бани ", "Салон красоты 'Loris'", "Доставка еды 'БыстроСуши'",]
...
time = Benchmark.measure do
  owner = Partner.find_by(email: Rails.application.credentials.email[:demo_user])
  #create_towns
  #create_categories

  Property.destroy_all
  65.times { create_property_for(owner) }
  # Property.reindex
  # puts "Properties indexed"
  
  Activity.destroy_all
  35.times { create_activity_for(owner) }

  Service.destroy_all
  35.times { create_service_for(owner) }

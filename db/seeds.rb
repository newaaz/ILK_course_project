...
SERVICE_TITLES = ["Такси 'Девяточка'", "Служба такси 'Виктор'", "Детская комната 'Одуванчик'", "Ремонт обуви", "Ремонт смартфонов",
...
time = Benchmark.measure do
  ...
  26.times { create_property_for(owner) }
  # Property.reindex
  # puts "Properties indexed"
  
  Activity.destroy_all
  6.times { create_activity_for(owner) }

  Service.destroy_all
  6.times { create_service_for(owner) }

  Place.destroy_all
  6.times { create_place_for(owner) }

  FoodPlace.destroy_all
  6.times { create_food_place_for(owner) }
end

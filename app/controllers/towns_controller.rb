class TownsController < ApplicationController
  ...
  def show
    ...
    @promo_properties = @town.properties.activated.where(rating: 15).take(6)
    @promo_activities = @town.activities.activated.where(rating: 15).take(6)

    @promo_services = @town.services.activated.where(rating: 15).take(6)
    #@taxi_services = @promo_services.select { |service| service.rating == 16 }
    #@food_delivery_services = @promo_services.select { |service| service.rating == 17 }
    @taxi_services = @town.services.activated.where(rating: 16, category_title: 'Такси').take(2)
    @food_delivery_services = @town.services.activated.where(rating: 16, category_title: "Доставка еды").take(2)

    @promo_food_places = @town.food_places.activated.where(rating: 15).take(6)
    @promo_places = @town.places.activated.where(rating: 15).take(6)
  end
